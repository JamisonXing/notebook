...
Java 集合， 也叫作**容器**，主要是由两大接口派生而来：一个是 `Collection`接口，主要用于存放单一元素；另一个是 `Map` 接口，主要用于存放键值对。对于`Collection` 接口，下面又有三个主要的子接口：`List`、`Set` 和 `Queue`。
...
​	现在的程序是实例化了Book类的对象，和给出的MathBook类没有任何的联系，所以MathBook类是否存在与本程序是没有联系的，那么如果说此时采用的是子类进行对象的实例化呢？
...
Result: [MathBook]这是一本书

​	因为此时实例化的是MathBook子类，并且在MathBook子类中也成功进行了getInfo方法的覆写，所以调用的时候一定要找的是被覆写过的方法。

![image-20230118111243111](/Users/jamison/Library/Application Support/typora-user-images/image-20230118111243111.png)

​	通过以上分析发现，new决定了对象实例所属的类型，也决定了最终执行的方法是属于哪个类中的方法，那么依据这种概念来观察对象的向上转型。
...
![image-20230118111800412](/Users/jamison/Library/Application Support/typora-user-images/image-20230118111800412.png)

**范例：基于方法重载实现当前的开发要求**

![image-20230118111857448](/Users/jamison/Library/Application Support/typora-user-images/image-20230118111857448.png)

![image-20230118111926783](/Users/jamison/Library/Application Support/typora-user-images/image-20230118111926783.png)

**范例：利用对象的向上转型来解决当前的设计问题**
...
public class JamisonDemo {
    ...
    public static void fun(Book book) { //介绍book子类实例
        System.out.println(book.getInfo());
    }
}
```
